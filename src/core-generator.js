...
(function (root) {
  ...
  /* ---------- Ausgabedateien ----------------------------------------- */
  // ÖFFENTLICH (Upload zur Scanner-Seite): NUR Token + Ticket-Nr., keine PII.
  function tokensJson(records) {
    return records.map(function (r) {
      return { token: r.token, ticketNr: r.ticketNr, ticketAnzahl: r.ticketAnzahl };
    });
  }
  ...
  // LOKAL (bleibt beim Organisator, NICHT hochladen): Token -> Klarname.
  // Dient dem späteren Abgleich (auswertung.html) und als Outlook-Referenz.
  function zuordnungCsv(records) {
    var d = ';', lines = ['token' + d + 'Nachname' + d + 'Vorname' + d + 'Studiengruppe' + d + 'Studienort' + d + 'Einstellungsjahr' + d + 'TicketNr' + d + 'Anzahl' + d + 'Dateiname'];
    records.forEach(function (r) {
      lines.push([r.token, csvFeld(r.nachname, d), csvFeld(r.vorname, d), csvFeld(r.studiengruppe, d), csvFeld(r.studienort, d), csvFeld(r.einstellungsjahr, d), r.ticketNr, r.ticketAnzahl, csvFeld(r.dateiname, d)].join(d));
    });
    return '﻿' + lines.join('\r\n') + '\r\n';
  }
  ...
  function baueAlles(opts) {
    ...
    return p.then(function () {
      ...
      zip.file('zuordnung.csv', zuordnungCsv(meta));
  ...
  TG.tokensJson = tokensJson; TG.zuordnungCsv = zuordnungCsv;
